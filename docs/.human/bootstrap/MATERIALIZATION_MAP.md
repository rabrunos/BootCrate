...
For existing code, preserve useful behavior/instructions in a separate Adoption Sandbox and apply only the approved delta after checking the original baseline. For new products, choose minimal native capabilities. In both cases require Issues, a canonical version, selected security and relevant checks. Validate behavior and negative/security paths. Promote selected project rules/tests, then remove `docs/.human/bootstrap/`, its raw intake and `.github/workflows/bootcrate-validate.yml`. Rewrite the README and remove unused adapters. Run the post-materialization structural verifier from retained tools, then the resulting project's own checks; these prove different things. Confirm the target version, perform authorized Git/Issue actions and return the ephemeral report. If Console was selected, copy only its reduced static files before pruning.
