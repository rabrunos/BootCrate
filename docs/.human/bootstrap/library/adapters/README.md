...
Codex Protected Auto prepares `workspace-write` and `on-request` through the portable CLI flags and requests `approvals_reviewer = "auto_review"` through native settings. The CLI mapping does not depend on `--approve-for-me`: availability differs across clients. Its declared CLI support is `unknown` until an observation for that surface records `capabilities.approvals_reviewer_auto_review: true` and confirms the requested profile was effective. Record the observed client version as evidence if useful, without deriving support from a version range. Explicitly unsupported capability blocks Protected Auto; missing evidence leaves it unknown. Neither case switches to Full Access.

For Claude Full Access observations, the resolver requires an explicit matching `surface` and `capabilities.approval_bypass`, `capabilities.filesystem_unrestricted` and `capabilities.network_unrestricted` to be explicitly true before it reports `effective: full_access`. A flag or requested profile alone is not application evidence. Missing capabilities keep the result unknown, and a managed profile restriction blocks it even when an observation is supplied.
