(() => {
  ...
  function versionContract(profile) {
    ...
    if(profile.schema==="project-profile/v2") {
      const suffix=versioning.canonical_source.split(".").at(-1).toLowerCase();
      return {reader:suffix==="json"?"json":suffix==="toml"?"toml":"plain",valuePath:undefined,
        historySource:"CHANGELOG.md",historyFormat:"markdown-headings"};
    }
    return {reader:versioning.reader,valuePath:versioning.value_path,
  ...
  $("history").onchange=async()=>{try{if(!state.profile)throw new Error("Import a profile first");const contract=versionContract(state.profile);if(contract.historyFormat!=="markdown-headings")throw new Error("History reader '"+String(contract.historyFormat)+"' is not supported in this Console view; use native validation");if($("history").files[0]?.name!==contract.historySource.split("/").at(-1))throw new Error("Select the declared history source: "+contract.historySource);const source=await read("history");if(source===null)return;const entries=[...source.matchAll(/^##[ \t]+\[?v?([0-9A-Za-z][0-9A-Za-z._+-]{0,127})\]?(?:[ \t]|$|[\u2014:\u2013-])[^\r\n]*$/gm)];if(!entries.length||new Set(entries.map(e=>e[1])).size!==entries.length)throw new Error("No unique version headings found");commit({...state,history:entries.map(e=>e[0].replace(/^##[ \t]+/,""))});status("Version headings imported; run the native changelog parser before delivery.");}catch(error){status("History import failed: "+error.message+". Previous state was preserved.");}};
  ...
  $("validateLocal").onclick=()=>{const missing=[];if(!state.profile)missing.push("stable profile");else{if(!state.version)missing.push("canonical version source file");if(!state.history)missing.push((state.profile.versioning.history_source||"CHANGELOG.md")+" version headings");if(state.profile.distribution?.mode!=="none"&&!state.preflight)missing.push("per-destination preflight");}$("localCheck").textContent=missing.length?"Local preview incomplete: "+missing.join(", ")+". Full schema and product checks require native tools.":"Selected local inputs are present. Full schema, version consistency, product behavior and remote status still require native checks.";};
