(() => {
  ...
  function versionContract(profile) {
    const versioning=profile.versioning;
    if(profile.schema==="project-profile/v2") {
      // v2 has no declared reader, value path or history source. Keep only its old UI previews.
      return {legacy:true,source:versioning.canonical_source};
    }
    return {legacy:false,reader:versioning.reader,valuePath:versioning.value_path,
      historySource:versioning.history_source,historyFormat:versioning.history_format};
  }
  function atPointer(value,pointer) {
    if(typeof pointer!=="string"||!pointer.startsWith("/"))throw new Error("JSON reader requires a declared value path");
    return pointer.slice(1).split("/").reduce((current,part)=>{
      const key=part.replace(/~1/g,"/").replace(/~0/g,"~");
      if(Array.isArray(current)&&/^\d+$/.test(key)&&Number(key)<current.length)return current[Number(key)];
      if(current===null||typeof current!=="object"||!Object.hasOwn(current,key))throw new Error("Version value path not found");
      return current[key];
    },value);
  }
  function versionValue(raw,contract) {
    let value;
    if(contract.legacy) {
      if(contract.source.split("/").at(-1)==="VERSION")value=raw.trim();
      else if(contract.source.endsWith(".json"))value=JSON.parse(raw)?.version;
      else throw new Error("Legacy v2 source '"+contract.source+"' cannot be interpreted in this Console view; use native validation");
      if(typeof value!=="string"||!(/^[0-9A-Za-z][0-9A-Za-z.+-]{0,63}$/.test(value)))
        throw new Error("Legacy v2 version source has no readable root version value; use native validation");
      return value;
    }
  ...
  $("version").onchange=async()=>{try{if(!state.profile)throw new Error("Import a profile first");const rawFile=await read("version");if(rawFile===null)return;const expected=state.profile.versioning.canonical_source.split("/").at(-1);if($("version").files[0]?.name!==expected)throw new Error("Selected file does not match the declared version source");const value=versionValue(rawFile,versionContract(state.profile));commit({...state,version:value});status("Version source imported; this reflects the selected local file only.");}catch(error){status("Version import failed: "+error.message+". Previous state was preserved.");}};
  $("history").onchange=async()=>{try{if(!state.profile)throw new Error("Import a profile first");const contract=versionContract(state.profile);if(!contract.legacy&&contract.historyFormat!=="markdown-headings")throw new Error("History reader '"+String(contract.historyFormat)+"' is not supported in this Console view; use native validation");const historySource=contract.legacy?"CHANGELOG.md":contract.historySource;if($("history").files[0]?.name!==historySource.split("/").at(-1))throw new Error(contract.legacy?"Select the optional legacy CHANGELOG.md":"Select the declared history source: "+historySource);const source=await read("history");if(source===null)return;const entries=[...source.matchAll(/^##[ \t]+\[?v?([0-9A-Za-z][0-9A-Za-z._+-]{0,127})\]?(?:[ \t]|$|[\u2014:\u2013-])[^\r\n]*$/gm)];if(!entries.length||new Set(entries.map(e=>e[1])).size!==entries.length)throw new Error("No unique version headings found");commit({...state,history:entries.map(e=>e[0].replace(/^##[ \t]+/,""))});status(contract.legacy?"Optional legacy changelog headings imported; v2 declares no history source. Use native validation.":"Version headings imported; run the native changelog parser before delivery.");}catch(error){status("History import failed: "+error.message+". Previous state was preserved.");}};
  ...
  $("validateLocal").onclick=()=>{const missing=[];if(!state.profile)missing.push("stable profile");else{if(!state.version)missing.push("readable canonical version source file");if(state.profile.schema==="project-profile/v3"&&!state.history)missing.push(state.profile.versioning.history_source+" version headings");if(state.profile.schema==="project-profile/v3"&&state.profile.distribution.mode!=="none"&&!state.preflight)missing.push("per-destination preflight");}const legacy=state.profile?.schema==="project-profile/v2"?" Legacy v2 declares no reader or history source; this preview cannot verify their contents.":"";$("localCheck").textContent=missing.length?"Local preview incomplete: "+missing.join(", ")+"."+legacy+" Full schema and product checks require native tools.":"Selected local inputs are present."+legacy+" Full schema, version consistency, product behavior and remote status still require native checks.";};
